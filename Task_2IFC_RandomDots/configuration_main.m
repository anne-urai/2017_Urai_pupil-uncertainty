function [setup, dots, fix, results, sound, flip] = configuration_main(window, audio, setup)
...
setup.feedbackcounterbalance = setup.feedbackcounterbalance(setup.participant);

% see if we can find a file upon which to base the perfrange
try
    load(sprintf('D:\USERS\AnneUrai\OjayMedina\Data\P%02d_threshold.mat', setup.participant));
    if threshold < 0.10,
        setup.perfrange = 'hard';
    elseif threshold < 0.15,
        setup.perfrange = 'medium';
    elseif threshold > 0.15,
        setup.perfrange = 'easy';
    end
catch
    % first session, no threshold file available yet
    setup.perfrange = 'easy';
end

% determine the coherence levels
...
setup.trialrep          = 100; %nr of trials per coherence level
...
setup.viewingtime       = .5; % viewing duration in seconds (fixed in this script, or maximum viewing duration in RT paradigm
...
setup.pupilreboundtime  = (1.5 + 1 * rand(setup.nblocks, setup.ntrials)); %wait average 4 seconds before displaying feedback
setup.pupilreboundtime2 = (2 + .5 * rand(setup.nblocks, setup.ntrials));
setup.ISI               = 1;
...
setup.difficulty        = setup.coherence;
setup.difficulty(abs(setup.difficulty - setup.cohlevels(1)) < 0.001) = 1;
setup.difficulty(abs(setup.difficulty - setup.cohlevels(2)) < 0.001) = 2;
setup.difficulty(abs(setup.difficulty - setup.cohlevels(3)) < 0.001) = 3;
setup.difficulty(abs(setup.difficulty - setup.cohlevels(4)) < 0.001) = 4;
setup.difficulty(abs(setup.difficulty - setup.cohlevels(5)) < 0.001) = 5;
