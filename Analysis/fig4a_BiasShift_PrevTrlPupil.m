...
for whichLag = 1,
    ...
    spcnt = spcnt + 1;
    clearvars -except whichMod spcnt sj subjects nbins whichLag;
    ...
    for sj = unique(subjects),
        ...
        switch whichMod
            case 'rt'
                data.rt = log(data.rt);
            case 'baseline_pupil'
                % take the baseline on the same trial as we're fitting the
                % logistic function!
    ...
    for ri = 1:2,
        for ui = 1,
            plot(stimx, squeeze(mean(grandavg.curve(:, ri, ui, :))), 'color', colors(cnt, :));
            cnt = cnt + 1;
        end
    end
    ...
    text(0, 0.55, sprintf('Main modul F_{(%d,%d)} = %.2f', stats.f2.df(1), stats.f2.df(2), stats.f2.fstats));
    text(0, 0.40,  sprintf('p = %.3f', stats.f2.pvalue));
...
%% save fig
print(gcf, '-dpdf', sprintf('~/Dropbox/Figures/uncertainty/Fig4_valueShift_allTrials_%s.pdf', whichMod));
