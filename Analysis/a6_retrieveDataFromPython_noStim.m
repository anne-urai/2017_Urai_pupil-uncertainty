...
for sj = subjects,
    ...
    bh = boundedline(lags, respw, respwci, 'cmap', colors([4], :), 'alpha');
    legend(bh, 'response'); legend boxoff;
    xlim([0.5 nlags+0.5]);
    ...
    switch whichmodulator
        ...
        otherwise % only do when we ran a modulatory model
            ...
            % also get error bars, multiply bootstrapped values with slope too
            pupilrespboot = bootstrap_corr(:, nlags*2+1:nlags*3);
            ...
            bh = boundedline(lags, pupilrespw, pupilrespwci, 'cmap', colors([4], :), 'alpha');
            legend(bh, [whichmodulator '*response']); legend boxoff;
            xlim([0.5 nlags+0.5]);
            ylabel('Interaction');
    ...
    print(gcf, '-dpdf', sprintf('~/Dropbox/Figures/serial/historykernels_%sNoStim_sj%02d.pdf', whichmodulator, sj));
    
end

% save for group plots
savefast(sprintf('~/Data/pupilUncertainty/GrandAverage/historyweights_%s_nostim.mat', whichmodulator), 'dat');
close all;