...
myMdl.rt = glmer(resp ~ 1 + motionstrength + prevResp + prevRT + prevResp*prevRT  
                 + (1 + motionstrength + prevResp + prevRT + prevResp*prevRT|subjnr/sessionnr), 
                 family = binomial, verbose=0, data = data)
summary(myMdl.rt)
anova(myMdl.history, myMdl.rt) # does the rt model do better than history?
anova(myMdl.pupil, myMdl.rt) # which of the two uncertainty measures fits the data better?
